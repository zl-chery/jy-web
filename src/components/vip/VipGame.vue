        ...
        <div>
          <ul class="jy-history--list jy-vip--nomargin">
            <li v-for="item in freeGame" :key="item.title">
              <router-link  :to="{path:'/gamePool',query:{id:item.jump_id}}" class="jy-historylist--info">
...
export default {
  ...
  mounted: async function () {
    ...
    this.$nextTick(() => {
      setInterval( () => {
        ...
        this.date.day = 7 - nowDate.getDay()
        this.date.hour = 24 - nowDate.getHours()
        this.date.minute = 60 - nowDate.getMinutes()
        this.date.second = 60 - nowDate.getSeconds()
      }, 1000)
    })
  },
  methods: {
  ...
  filters: {
    ...
    paddingLeft(str) {
      return String(str)[1] &&  String(str) || '0' + str
    }
  },
}
</script>
