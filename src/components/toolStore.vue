...
import HDCardVue from './toolstore/HDCard.vue';
export default {
  name: 'ToolStore',
  props: {
    msg: String
  ...
  components: {
    AllCard, SecondCardVue, VipCardVue, HDCardVue
  },
  data() {
    return {
      activeName: 'first',
      ...
      keepComponents: ['AllCard', 'SecondCardVue', 'VipCardVue', 'HDCardVue'],
      label: [
        {
          label: "全部",
          comp: AllCard,
  ...
  methods: {
    ...
    changeComponent(value) {
      ...
      this.$router.push({
        path: `/toolStore/${this.label[this.activeIndex].path}`,
        // path: `/toolStore/${this.label[this.activeIndex].path}`,
        // query: {
        //   id: id
        // }
