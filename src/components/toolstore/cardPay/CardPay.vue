  ...
  <el-dialog
    ...
    <div class="jy-toolpay--show">
      <div class="jy-toolpay--title">
        <p class="jy-toolstore--pay">{{list.name | sliceValue}}</p>
        <img src="~@/assets/img/toolpay-cancel.png" alt="取消"   @click="toVisabled(list.index)">
      </div>
      <div class="jy-toolpay--info">
        <img :src="list.img_url" alt="">
        <div class="jy-toolpay--des">
          <p>{{list.name}}</p>
          <p>{{list.valid_hours}}小时内免进</p>
        </div>
    ...
    <div class="jy-toolpay--content jy-card--show">
      <div class="jy-toopay--row">
        ...
        <div class="jy-toolpay-inputnum">
          <el-input-number class="toolpay-inputnum" v-model="num" @change="handleChange" :min="1" :max="100" label="描述文字"></el-input-number>
        </div>
      </div>
      <div class="jy-pay--method3 jy-toolpay--method">
        ...
        <div class="jy-qr--code">
          ...
          <div class="jy-pay--money jy-qr-money">
            <p>您需支付：<span>{{totalPrice}}</span>元</p>
...
export default {
  ...
  // computed: {
  //   dialog: function () {
  //     return this.list.dialogVisible
  //   },
  //   list: function() {
  //     return  this.item
  //   }
  ...
  computed:{
    totalPrice() {
      let totalPrice = 0
       totalPrice = this.list.price * this.num
       return totalPrice
    }
  },
  watch:{
  ...
  data() {
    return {
      num: 1,  //购买数量
      list: this.item, // list数据
      dialog: this.dialogVisible,  //dialog是否可见
...
.jy-toolpay--content.jy-card--show
  margin-top 25%
